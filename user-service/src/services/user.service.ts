...
export class UserService {
    ...
    */
     ...
     */
    async getAdvertisingDetailsByIds(userIds: (string | Types.ObjectId)[]): Promise<any[]> {
        try {
            const objectIds = userIds.map(id => typeof id === 'string' ? new Types.ObjectId(id) : id);
            return await userRepository.findAdvertisingDetailsByIds(objectIds);
        } catch (error: any) {
            log.error(`Error fetching advertising user details by IDs: ${error.message}`, { userIds });
            throw new Error('Failed to fetch advertising user details');
        }
    }
